...
namespace OLSR.OLSR.RoutingTable
{
    ...
    /// 
    /// </summary>
    public class RoutingTableCalculation
    {
        ...
        public void ModifyRoutingTable(ArrayList pRoutes)
        {
            ...
            else
            {
                lock (OLSRParameters.RoutingSet)
                {
                    ...
                    if (!ChangeRoutingTable.SearchRoute(tmpSt))
                    {
                        foreach (Route route in OLSRParameters.RoutingSet)
                        {
                    ...
                    if(ChangeRoutingTable.SearchRepeatRoutes(tmpSt))
                        ChangeRoutingTable.DeleteRoute(tmpSt);

                    //bool exist = false;                   
                    //foreach (Route route in OLSRParameters.RoutingSet)
                    //{
                    //    if (route.R_dest_addr_.ToString().EndsWith(".1"))
                    //    {
                    //        exist = true;
                    //        break;
                    //    }
        ...
        /// <summary>
        /// Metodo encargado de comprobar si existe un ruta en la tabla de rutas del sistema
        /// </summary>
        /// <param name="pRoute">Nueva ruta</param>
        /// <param name="pRoutes">Listado de rutas del sistema</param>
        /// <returns>Boolean indicando si esxiste o no la ruta</returns>
